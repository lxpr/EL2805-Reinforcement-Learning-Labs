...
from DDPG_agent import ActorNeuralNet, ActorNet
...
def check_solution(number):
    # Load model
    try:
        model = torch.load(f'Networks/actor-network-{number}.pth')
        print('Network model: {}'.format(model))
    except:
        print(f'File actor-network-{number}.pth not found!')
        exit(-1)
    ...
    if avg_reward - confidence >= CONFIDENCE_PASS:
        ...
        return "Passed", avg_reward, confidence
    else:
        ...
        return "Failed", avg_reward, confidence
