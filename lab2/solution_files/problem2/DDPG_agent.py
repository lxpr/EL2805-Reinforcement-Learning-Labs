...
class DDPGAgent(Agent):
    ...
    def learn(self, combined=False, actor_update=False):
        ...
        next_s = torch.tensor(next_states, requires_grad=False, dtype=torch.float32)
        ...
        r = torch.tensor(rewards, requires_grad=False, dtype=torch.float32).reshape(-1, 1)
        ...
        loss.backward()
        nn.utils.clip_grad_norm_(self.critic.parameters(), max_norm=self.clip_val)
        ...
        if actor_update:
            ...
            nn.utils.clip_grad_norm_(self.actor.parameters(), max_norm=self.clip_val)
            self.opt_actor.step()
