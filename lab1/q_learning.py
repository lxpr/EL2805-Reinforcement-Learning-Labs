...
class QLearningAgent:
    def __init__(self, env, alpha=None, gamma=None, epsilon=None):
        ...
        self.P = env.transition_probabilities
    ...
    def learn(self, state, action, reward, state2):
        ...
        self.Q[state, action] = self.Q[state, action] + 1 / (self.N[state, action] ** self.alpha) * (reward + self.gamma * max(self.Q[state2, :]) - self.Q[state, action])